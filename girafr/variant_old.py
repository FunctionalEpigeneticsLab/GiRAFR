...
def call_variants_from_consensus(consensus_bam_in = 'consensus.bam', ref_fasta = 'oligo_pool_plasmid.fa', structure_gtf = 'oligo_pool_plasmid_structure.gtf'):

	""" Call variants from consensus sequence of gRNA library and save into variants dictionary
	Arg:
	...
	Return:
		variants = {} 2d dictionary: variants[gene][seq] = [n, cigar, nM, umi_count, ref_seq, int(min(ref_pos))]
	"""
	...
	for r in bam_in:
		...
		if (is_10x == False) & (utils.get_read_tag(r,'MD') != None):
...
def call_gRNA_variant(output_dir, consensus_seq_file = 'consensus.sequence.gRNA.txt', ref_fasta = 'oligo_pool_plasmid.fa', structure_gtf = 'oligo_pool_plasmid_structure.gtf'):
	...
	variants = call_variants_from_consensus(consensus_bam_in = output_dir + 'consensus.bam', ref_fasta = ref_fasta, structure_gtf = structure_gtf) 
	...
	with open(consensus_seq_file, 'r') as consensus_seq_in:
		for l in consensus_seq_in:
			...
			try:
				...
				print(l, gene, seq)
				exit()
			out_line = l + "\t" + gene + "_variant_" + str(variants[gene][seq][0])+":"+mismatch + "\t" + (',').join(mismatch_structure) + "\n"
			out.write(out_line)
